...
/** 包内自带素材目录：lib/ 的上一级 assets/。 */
const BUNDLED_DIR = join(dirname(fileURLToPath(import.meta.url)), '..', 'assets')
...
/** 读取持久化配置，坏文件回落到默认值。 */
function readConfig() {
  const fallback = { mode: 'c', dim: 0.45, source: '', dynamic: false, panel: 'handle' }
  try {
...
function writeConfig(patch) {
  ...
  if (typeof next.mode === 'string' && !MODES.has(next.mode)) next.mode = 'c'
  if (typeof next.dynamic !== 'boolean') next.dynamic = false
...
/** 以人为主体的素材：排在列表最下方，风景类排在前面。 */
const PEOPLE_BASES = new Set(['silent-blade-samurai', 'shadow-knight-vs-titan'])
...
/** 首帧静帧是同名 .poster.jpg，属于附属文件，不进素材列表。 */
function isPosterFile(name) {
  return /\.poster\.jpg$/i.test(name)
}

/** 读一个目录里的媒体文件名（读不到就当空目录）。 */
function readMediaDir(dir) {
  try {
    return readdirSync(dir).filter((f) => MIME[extname(f).toLowerCase()] !== undefined
      && f !== 'wallpaper.json' && !isPosterFile(f))
  } catch {
    return []
  }
}
...
/**
 * 列出可用素材，并给出显示名。
 * 包内 assets/ 与 ~/.dsh/theme-assets/ 同时扫描，同名文件以**包内**为准。
 * 同一基名的文件共用显示名（不加任何画质后缀）。
 * 排序：风景类在前（按显示名），以人为主体的素材垫底。
 * @returns [{ file, label, size }]
 */
function listAssets() {
  const seen = new Set()
  const entries = []
  // 同名时 **用户目录优先**：本地放了自己的版本就以本地为准，包内素材只作默认值。
  for (const dir of [ASSET_DIR, BUNDLED_DIR]) {
    for (const name of readMediaDir(dir)) {
  ...
  for (const [base, items] of groups) {
    ...
    for (const item of items) rows.push({ file: item.file, label, size: item.size })
  }
  rows.sort((a, b) => {
    const peopleA = PEOPLE_BASES.has(assetBase(a.file)) ? 1 : 0
    const peopleB = PEOPLE_BASES.has(assetBase(b.file)) ? 1 : 0
    return peopleA - peopleB || a.label.localeCompare(b.label)
  })
  return rows
}

/** 在素材目录内解析请求路径（用户目录优先，与 listAssets 同序），两边都挡目录穿越。 */
function resolveAsset(urlPath) {
  const rel = decodeURIComponent(urlPath.replace(/^\/wallpaper-assets\/?/, ''))
  if (rel === '' || rel.includes('\0')) return null
  for (const dir of [ASSET_DIR, BUNDLED_DIR]) {
    const abs = resolve(join(dir, normalize(rel)))
...
export function apply(ctx) {
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: (req, res) => {
      ...
      // 校验器：Cache-Control: no-cache 的语义是"用前必须校验"，
      // 缺了校验器浏览器就只能整段重下。由 size + mtime 派生，文件一改自动失效。
      ...
      if (typeof inm === 'string' && (inm.trim() === '*' || inm.split(',').some((tag) => tag.trim() === etag))) {
        res.writeHead(304, validators)
        res.end()
        return
      }
      // If-Range 不匹配说明文件已变，此时不能续传旧分片，退回整段响应。
      const ifRange = req.headers?.['if-range']
      const range = (ifRange === undefined || ifRange === etag) && typeof req.headers?.range === 'string'
        ? req.headers.range
        : undefined
