...
import { CATALOG, CATEGORIES, catalogForUi, downloadToAssets, findDownloaded, sniffCodec, fileSizeMB } from './download.js'
...
export function apply(ctx) {
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: (req, res) => {
      if (reject(req, res)) return
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' })
      res.end(JSON.stringify({ ok: true, items: catalogForUi(), categories: CATEGORIES, job }))
    },
        ...
        void (async () => {
          try {
            ...
            if (job !== null) {
              ...
              job.result = { path, file: basename(path), codec, mb: Number(fileSizeMB(path).toFixed(1)) }
            }
          } catch (err) {
            if (job !== null) {
              job.state = 'error'
