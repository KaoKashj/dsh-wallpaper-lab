...
/* 显示层与播放层分离：video 真的播起来才显示视频，否则显示 poster 静帧。
   这样自动播放被拦时用户看到的是画面，而不是一个播放键。 */
#dsh-wall video { visibility: hidden; }
...
(function () {
  ...
  function renderModeC(host) {
    ...
    /** 取第一帧作为静帧背景（海报层）。取到就立刻有画面，不依赖能否自动播放。 */
    function makePoster() {
  ...
  function panel(mode) {
    ...
    bar.appendChild(dlSel);   // ← 之前漏了这一行，下拉框创建了但没进界面
      ...
      fetch('/wallpaper-catalog').then(function (r) { return r.json(); }).then(function (d) {
        ...
        order.forEach(function (key) {
          ...
          groups[key].forEach(function (it) {
            ...
            opt.textContent = it.name + sizeTag + (it.downloaded ? ' ✓' : '');
            group.appendChild(opt);
          });
