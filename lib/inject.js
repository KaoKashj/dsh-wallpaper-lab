...
(function () {
  ...
  // 内置可下载清单（由 refreshCatalog 填充），供 syncUI 追加到选择框
  var catalogItems = [];
  var catalogCategories = [];

  /** 把内置清单追加到选择框，分组名「可下载 · xxx」。 */
  function appendCatalogToSelect(s) {
    if (!s || !catalogItems.length) return;
    var catName = {};
    catalogCategories.forEach(function (c) { catName[c.id] = c.name; });
    var order = [], groups = {};
  ...
  function syncUI() {
    ...
    if (!sel) return;
      ...
      if (catalogItems.length) {
        // 本机还没有素材，但内置清单可以下 —— 仍然让用户能选
        sel.disabled = false;
        appendCatalogToSelect(sel);
      } else {
    ...
    // 本机素材列完，再把内置清单追加为「可下载」分组 ——
    // syncUI 会清空整个 select，所以这一步必须放在最后。
    appendCatalogToSelect(sel);
  }
  ...
  function panel(mode) {
    ...
    sel.title = '选择壁纸：本机素材直接切换；带 ⇣ 的会先下载再切换';
    sel.addEventListener('change', function () {
      var v = sel.value;
      // 内置清单里的条目用 'want:' 前缀，避免和真实文件名撞车
      if (v.indexOf('want:') === 0) {
        var id = v.slice(5);
        var entry = null;
        ...
        if (entry && entry.downloaded) {
          ...
          Array.prototype.forEach.call(sel.options, function (o) {
            if (o.value.indexOf(id) === 0 && o.value.indexOf('want:') !== 0) mine = o.value;
          });
          if (mine) { switchSource(mine); return; }
        }
        startDownload(id);
        return;
      }
        ...
        if (j.state === 'done') {
          ...
          if (r.file) {
            fetch('/wallpaper-mode', {
              ...
              setTimeout(function () { refreshCatalog(); }, 600);
    ...
    /**
     * 下载某个内置壁纸（id）。已下载过的不会走到这里。
     * 下完由 pollJob 自动切换为当前壁纸。
     */
    ...
    function refreshCatalog() {
      fetch('/wallpaper-catalog').then(function (r) { return r.json(); }).then(function (d) {
        ...
        catalogCategories = d.categories || [];
        syncUI();   // 由 syncUI 统一渲染（它会带上「可下载」分组）
    ...
    dlBtn.addEventListener('click', function () {
      // 按钮：下载主下拉框里当前选中的内置条目（若选的是本机素材则提示）
      var v = sel.value;
      if (v.indexOf('want:') === 0) startDownload(v.slice(5));
      else dlHint.textContent = '请在左侧列表里选一张带 ⇣ 的壁纸';
    });
