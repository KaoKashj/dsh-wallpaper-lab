...
(function () {
  ...
  function renderModeC(host) {
    ...
    // muted / playsinline 必须在设 src 之前落定，否则 Chrome 可能判定"不允许自动播放"，
    // 于是停在暂停态 —— 那正是画面中央出现播放三角的原因。
    v.muted = true;
    v.defaultMuted = true;
    ...
    // 只要标签页可见就应当播放（不再因"窗口失焦"暂停 —— 那会留下播放三角且回不来）
    var play = function () {
      if (videoEl !== v) return;
    ...
    ['loadeddata', 'canplay', 'canplaythrough'].forEach(function (evt) {
      v.addEventListener(evt, play);
    });
    document.addEventListener('visibilitychange', function () {
      if (videoEl !== v) return;
      if (document.visibilityState === 'hidden') { try { v.pause(); } catch (e) {} } else play();
    });
    // 兜底：任何一次用户交互（点击/按键/滚轮）后强制恢复播放
    ['pointerdown', 'keydown', 'wheel'].forEach(function (evt) {
      document.addEventListener(evt, play, { passive: true });
    });
    play();

    // 兜底：10 秒后仍处于暂停（浏览器就是不让自动播放）→ 改用首帧静态图，
    // 这样画面上不会一直挂着那个播放三角。
    setTimeout(function () {
      if (videoEl !== v || !v.paused || v.readyState < 2) return;
      try {
